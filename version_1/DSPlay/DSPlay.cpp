...
PlayList  cPlayList;
char	StrokeSelection[4];
char	CorrectStroke;
PlayList::tPlayListItem PlayItem;
LocalTimer MyTimer;
LogResults cLogResults;
BOOL gGameInProgress=false;
BOOL gFirstTimeThrough=true;
char	gBlankScreen[MAX_PATH];
...
HRESULT PlayMovieInWindow(LPTSTR szFile)
{
    ...
    if (!pszFile)
        return E_POINTER;
    wcsncpy(wFile, pszFile, NUMELMS(wFile)-1);
    wFile[MAX_PATH-1] = 0;
    ...
    if( SUCCEEDED( hr ) )
    {
        ...
        if (!g_bAudioOnly)
        {
            ...
            JIF(InitVideoWindow(4, 1));
			JIF(pVW->put_FullScreenMode((long) -1));
        }
        else
        {
        ...
        ShowWindow(ghApp, SW_MAXIMIZE ); // SW_SHOWNORMAL);//
        UpdateWindow(ghApp);
...
BOOL GetLogFileName(LPTSTR szName)
{
    static OPENFILENAME ofn={0};
    static BOOL bSetInitialDir = FALSE;
    ...
    *szName = 0;
    ...
    ofn.hwndOwner         = ghApp;
    ofn.lpstrFilter       = NULL;
    ofn.lpstrFilter       = FILE_SAVE_FILTER_TEXT;
    ofn.lpstrCustomFilter = NULL;
    ofn.nFilterIndex      = 1;
    ofn.lpstrFile         = szName;
    ofn.nMaxFile          = MAX_PATH;
    ofn.lpstrTitle        = TEXT("Save results file as...\0");
    ofn.lpstrFileTitle    = NULL;
    ofn.lpstrDefExt       = TEXT("*\0");
    ofn.Flags             = OFN_FILEMUSTEXIST | OFN_READONLY | OFN_PATHMUSTEXIST;
...
void Msg(TCHAR *szFormat, ...)
{
    ...
    MessageBox(NULL, szBuffer, TEXT("QTester"), MB_OK);
}
...
HRESULT HandleGraphEvent(void)
{
                    ...
                    (LONG_PTR *) &evParam2, 0)))
    {
        ...
        {
		  SendMessage(ghApp, WM_KEYDOWN, (int) '0', 0);
        }
...
LRESULT CALLBACK WndMainProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
		...
		case WM_LBUTTONUP:
			{
				PauseClip();
				DialogBox(ghInst, MAKEINTRESOURCE(IDD_ABOUTBOX), ghApp,  (DLGPROC) AboutDlgProc);
			}
			break;
        ...
        case WM_KEYDOWN:
			...
			if (gGameInProgress == true)
			{
				...
				if ((wParam == (WPARAM) toupper(StrokeSelection[0])) ||
					(wParam == (WPARAM) toupper(StrokeSelection[1])) ||
					(wParam == (WPARAM) toupper(StrokeSelection[2])) ||
					(wParam == (WPARAM) toupper(StrokeSelection[3])) ||
					(wParam == (WPARAM) '0'))
				{
					MyTimer.Stop();	
					StopClip();
					cLogResults.StoreResult (PlayItem.FileToPlay  , 
											 (char) wParam, 
											 (wParam == (WPARAM) toupper(CorrectStroke)),
											 MyTimer.TimeTaken());
					SendMessage(ghApp, WM_COMMAND, ID_BLANK_CLIP, 0);

				}
			}
			else
			{
				SendMessage(ghApp, WM_COMMAND, ID_NEXT_CLIP, 0);
			}
        case WM_COMMAND:
            ...
            { // Menus
				case ID_CREATE_WINDOW:
					if(GetCurrentDirectory(MAX_PATH,gBlankScreen) != 0)
					{
						strcat_s(gBlankScreen, MAX_PATH, "\\blank.bmp");
					}
					SendMessage(ghApp, WM_COMMAND, ID_FILE_OPENPLAYLIST, 0);
					break;
				...
				case ID_FILE_OPENPLAYLIST:
                    ...
                    if(GetLogFileName(filename) == true)
					{
						cLogResults.Start(filename);
						if(GetClipFileName(filename) == true)
						{
							cPlayList.ReadPlayList(filename);
							SendMessage(ghApp, WM_COMMAND, ID_NEXT_CLIP, 0);	
						}
						else
						{
				...
				case ID_BLANK_CLIP:
					...
					NextClip(gBlankScreen);
					break;
					
				case ID_NEXT_CLIP:
					gGameInProgress = true;
					cPlayList.NextItemOnPlayList(&PlayItem);
					if (strlen(PlayItem.FileToPlay) != 0)
					{
						if (gFirstTimeThrough == true)
						{
							char DynamicDialogText[1000] = {0};
							sprintf(DynamicDialogText, 
									"Prepare for Video,\nThe Valid Options for this Video are:\n1. %s => '%c'\n2. %s => '%c'\n3. %s => '%c'\n4. %s => '%c'\nClick OK, or press Enter when\nready for next video\n",
									PlayItem.OptionLabel[0], PlayItem.OptionCommand[0],                         
									PlayItem.OptionLabel[1], PlayItem.OptionCommand[1],
									PlayItem.OptionLabel[2], PlayItem.OptionCommand[2],
									PlayItem.OptionLabel[3], PlayItem.OptionCommand[3]);
							Msg(TEXT(DynamicDialogText));
							gFirstTimeThrough = false;
						}
						StrokeSelection[0] = PlayItem.OptionCommand[0];
						StrokeSelection[1] = PlayItem.OptionCommand[1];
						StrokeSelection[2] = PlayItem.OptionCommand[2];
						StrokeSelection[3] = PlayItem.OptionCommand[3];
						CorrectStroke = PlayItem.OptionCommand[PlayItem.CorrectOptionNumber-1];
						
						NextClip(PlayItem.FileToPlay);
					}
					else
					{
						cLogResults.Finish();
						gFirstTimeThrough = true;
						Msg(TEXT("The experiment has finished, and will now exit.\n"));
						SendMessage(ghApp, WM_COMMAND, ID_FILE_EXIT, 0);
					}
...
int PASCAL WinMain(HINSTANCE hInstC, HINSTANCE hInstP, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if(ghApp)
    {
        ...
        PostMessage(ghApp, WM_COMMAND, ID_CREATE_WINDOW, 0);
